...
region::parse_dimension_(const layout_node& mNode) {
    ...
    if (pAbsDimNode && pRelDimNode) {
        gui::out << gui::warning << mNode.get_location() << " : " << mNode.get_name()
        ...
        for (const auto& mAnchorNode : pAnchorsNode->get_children()) {
            if (mAnchorNode.get_name() != "Anchor" && mAnchorNode.get_name() != "") {
                gui::out << gui::warning << mAnchorNode.get_location() << " : "
                         << "unexpected node '" << mAnchorNode.get_name() << "'; ignored."
                         << std::endl;
                continue;
            }

            std::string sPoint =
                mAnchorNode.get_attribute_value_or<std::string>("point", "TOP_LEFT");
            std::string sParent = mAnchorNode.get_attribute_value_or<std::string>(
                "relativeTo", pParent_ || is_virtual() ? "$parent" : "");
            std::string sRelativePoint =
                mAnchorNode.get_attribute_value_or<std::string>("relativePoint", sPoint);
            ...
            if (utils::find(lFoundPoints, sPoint) != lFoundPoints.end()) {
                gui::out << gui::warning << mAnchorNode.get_location() << " : "
                         ...
                         << "\" has already been defined "
                            "for \""
                         << sName_ << "\". anchor skipped." << std::endl;
                continue;
            }

            anchor_data mAnchor(
                anchor::get_anchor_point(sPoint), sParent,
            ...
            if (const layout_node* pOffsetNode = mAnchorNode.try_get_child("Offset")) {
                ...
                mAnchor.mOffset  = vector2f(
                    mDimensions.second.x.value_or(0.0f), mDimensions.second.y.value_or(0.0f));
            }
