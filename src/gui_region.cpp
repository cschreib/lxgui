...
std::string region::serialize(const std::string& sTab) const {
    ...
    sStr << sTab << "  # Name        : " << sName_
         << " (" + std::string(bReady_ ? "ready" : "not ready") +
                std::string(bSpecial_ ? ", special)\n" : ")\n");
...
bool region::is_in_region(const vector2f& mPosition) const {
    return (
        (lBorderList_.left <= mPosition.x && mPosition.x <= lBorderList_.right - 1) &&
    ...
    if (sName_.empty()) {
        ...
        if (utils::starts_with(sName_, "$parent")) {
            if (pParent_)
                ...
                gui::out << gui::warning << "gui::" << lType_.back() << " : \"" << sName_
...
void region::set_parent_(utils::observer_ptr<frame> pParent) {
    if (pParent == observer_from_this()) {
        gui::out << gui::error << "gui::" << lType_.back() << " : Cannot call set_parent(this)."
                 << std::endl;
        return;
    }
...
void region::set_all_points(const std::string& sObjName) {
    if (sObjName == sName_) {
        gui::out << gui::error << "gui::" << lType_.back() << " : Cannot call set_all_points(this)."
                 << std::endl;
        return;
    }
...
void region::set_point(const anchor_data& mAnchor) {
    ...
    case anchor_point::TOP_LEFT:
        ...
        break;
    case anchor_point::TOP: lDefinedBorderList_.top = true; break;
    case anchor_point::TOP_RIGHT:
        lDefinedBorderList_.top   = true;
        lDefinedBorderList_.right = true;
        break;
    case anchor_point::RIGHT: lDefinedBorderList_.right = true; break;
    case anchor_point::BOTTOM_RIGHT:
        lDefinedBorderList_.bottom = true;
        lDefinedBorderList_.right  = true;
        break;
    case anchor_point::BOTTOM: lDefinedBorderList_.bottom = true; break;
    case anchor_point::BOTTOM_LEFT:
        lDefinedBorderList_.bottom = true;
        lDefinedBorderList_.left   = true;
        break;
    case anchor_point::LEFT: lDefinedBorderList_.left = true; break;
    default: break;
    }
...
void region::remove_anchored_object(region& mObj) {
    auto mIter =
        utils::find_if(lAnchoredObjectList_, [&](const auto& pPtr) { return pPtr.get() == &mObj; });
...
vector2f region::round_to_pixel(const vector2f& mPosition, utils::rounding_method mMethod) const {
    float fScalingFactor = get_manager().get_interface_scaling_factor();
    return vector2f(
        utils::round(mPosition.x, 1.0f / fScalingFactor, mMethod),
...
#define DEBUG_LOG(msg)
    ...
    if (!lAnchorList_.empty()) {
        ...
        float fRoundedWidth =
            round_to_pixel(mDimensions_.x, utils::rounding_method::NEAREST_NOT_ZERO);
        float fRoundedHeight =
            round_to_pixel(mDimensions_.y, utils::rounding_method::NEAREST_NOT_ZERO);
        ...
        if (pObj) {
            if (pObj->depends_on(*this)) {
                gui::out << gui::error << "gui::" << lType_.back()
                         ...
                         << "\"" << sName_ << "\" and \"" << pObj->get_name()
                         << "\" depend on "
                            "eachothers (directly or indirectly).\n\""
                         << anchor::get_string_point(mAnchor->mPoint) << "\" anchor removed."
                         << std::endl;
