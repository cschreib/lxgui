...
std::size_t count_frames(const std::array<strata, 8>& strata_list) {
    ...
    for (std::size_t ui_strata = 0; ui_strata < strata_list.size(); ++ui_strata) {
        for (const auto& m_level : utils::range::value(strata_list[ui_strata].level_list)) {
            ui_count += m_level.frame_list.size();
        }
    }
...
void print_frames(const std::array<strata, 8>& strata_list) {
    for (std::size_t ui_strata = 0; ui_strata < strata_list.size(); ++ui_strata) {
        if (strata_list[ui_strata].level_list.empty())
            continue;
        gui::out << "strata[" << ui_strata << "]" << std::endl;
        for (const auto& m_level : strata_list[ui_strata].level_list) {
            if (m_level.second.frame_list.empty())
                continue;
            gui::out << "  level[" << m_level.first << "]" << std::endl;
            for (const auto& p_frame : m_level.second.frame_list)
...
void frame_renderer::notify_strata_needs_redraw(frame_strata m_strata) {
    notify_strata_needs_redraw_(strata_list_[static_cast<std::size_t>(m_strata)]);
}
...
void frame_renderer::notify_frame_strata_changed(
    ...
    auto& m_old = strata_list_[static_cast<std::size_t>(m_old_strata)];
    auto& m_new = strata_list_[static_cast<std::size_t>(m_new_strata)];
...
void frame_renderer::notify_frame_level_changed(
    ...
    const auto m_frame_strata = p_frame->get_frame_strata();
    auto&      m_strata       = strata_list_[static_cast<std::size_t>(m_frame_strata)];
    auto&      level_list     = m_strata.level_list;

    auto m_iter_old = level_list.find(old_level);
    if (m_iter_old != level_list.end()) {
        remove_from_level_list_(m_iter_old->second, p_frame);
        if (m_iter_old->second.frame_list.empty())
            m_strata.level_list.erase(m_iter_old);
    }

    auto m_iter_new = level_list.find(new_level);
    if (m_iter_new == level_list.end()) {
        m_iter_new                  = level_list.insert(std::make_pair(new_level, level{})).first;
...
frame_renderer::find_topmost_frame(const std::function<bool(const frame&)>& m_predicate) const {
    ...
    for (const auto& m_strata : utils::range::reverse(strata_list_)) {
        for (const auto& m_level : utils::range::reverse_value(m_strata.level_list)) {
            for (const auto& p_frame : utils::range::reverse(m_level.frame_list)) {
...
int frame_renderer::get_highest_level(frame_strata m_frame_strata) const {
    const auto& m_strata = strata_list_[static_cast<std::size_t>(m_frame_strata)];
    if (!m_strata.level_list.empty())
        return m_strata.level_list.rbegin()->first;
...
void frame_renderer::add_to_strata_list_(
    ...
    auto m_iter_new = m_strata.level_list.find(new_level);
    if (m_iter_new == m_strata.level_list.end()) {
        m_iter_new = m_strata.level_list.insert(std::make_pair(new_level, level{})).first;
        m_iter_new->second.p_strata = &m_strata;
    }
...
void frame_renderer::remove_from_strata_list_(
    strata& m_strata, const utils::observer_ptr<frame>& p_frame) {
    auto m_iter = m_strata.level_list.find(p_frame->get_level());
    if (m_iter == m_strata.level_list.end()) {
    ...
    if (m_iter->second.frame_list.empty())
        m_strata.level_list.erase(m_iter);
}
...
void frame_renderer::remove_from_level_list_(
    ...
    auto m_iter = std::find(m_level.frame_list.begin(), m_level.frame_list.end(), p_frame);
    if (m_iter == m_level.frame_list.end()) {
        throw gui::exception("gui::frame_renderer", "frame not found in this strata and level");
    }
...
void frame_renderer::render_strata_(const strata& m_strata) const {
    for (const auto& m_level : utils::range::value(m_strata.level_list)) {
        for (const auto& p_frame : m_level.frame_list) {
            p_frame->render();
        }
    }
}
