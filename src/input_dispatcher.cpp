...
dispatcher::dispatcher(source& mSource) : mSource_(mSource) {
    ...
    lConnections_.push_back(
        mSource.on_mouse_pressed.connect([&](input::mouse_button mButton, gui::vector2f mMousePos) {
    ...
    lConnections_.push_back(
        mSource.on_mouse_wheel.connect([&](float fWheel, gui::vector2f mMousePos) {
            // Apply scaling factor to mouse coordinates
            mMousePos /= fScalingFactor_;
            // Forward
    ...
    lConnections_.push_back(
        mSource.on_mouse_moved.connect([&](gui::vector2f mMovement, gui::vector2f mMousePos) {
            // Apply scaling factor to mouse coordinates
...
double dispatcher::get_key_down_duration(key mKey) const {
    ...
    return std::chrono::duration<double>(
               timer::now() - lKeyPressedTime_[static_cast<std::size_t>(mKey)])
        .count();
}
...
double dispatcher::get_mouse_down_duration(mouse_button mID) const {
    ...
    return std::chrono::duration<double>(
               timer::now() - lMousePressedTime_[static_cast<std::size_t>(mID)])
        .count();
}
