...
void edit_box::parse_attributes_(const layout_node& node) {
    ...
    if (const layout_attribute* p_attr = node.try_get_attribute("letters"))
        set_max_letters(p_attr->get_value<std::size_t>());

    if (const layout_attribute* p_attr = node.try_get_attribute("blinkSpeed"))
        set_blink_speed(p_attr->get_value<float>());

    if (const layout_attribute* p_attr = node.try_get_attribute("numeric"))
        set_numeric_only(p_attr->get_value<bool>());

    if (const layout_attribute* p_attr = node.try_get_attribute("positive"))
        set_positive_only(p_attr->get_value<bool>());

    if (const layout_attribute* p_attr = node.try_get_attribute("integer"))
        set_integer_only(p_attr->get_value<bool>());

    if (const layout_attribute* p_attr = node.try_get_attribute("password"))
        enable_password_mode(p_attr->get_value<bool>());

    if (const layout_attribute* p_attr = node.try_get_attribute("multiLine"))
        set_multi_line(p_attr->get_value<bool>());

    if (const layout_attribute* p_attr = node.try_get_attribute("historyLines"))
        set_max_history_lines(p_attr->get_value<std::size_t>());

    if (const layout_attribute* p_attr = node.try_get_attribute("ignoreArrows"))
...
void edit_box::parse_all_nodes_before_children_(const layout_node& node) {
    ...
    if (const layout_node* p_highlight_node = node.try_get_child("HighlightColor"))
        set_highlight_color(parse_color_node_(*p_highlight_node));
}

void edit_box::parse_font_string_node_(const layout_node& node) {
    if (const layout_node* p_font_string_node = node.try_get_child("FontString")) {
        layout_node defaulted = *p_font_string_node;
        defaulted.get_or_set_attribute_value("name", "$parentFontString");

        auto p_font_string = parse_region_(defaulted, "ARTWORK", "FontString");
        if (p_font_string) {
            p_font_string->set_special();
            set_font_string(utils::static_pointer_cast<font_string>(p_font_string));
        }

        if (const layout_node* p_error_node = defaulted.try_get_child("Anchors")) {
            gui::out << gui::warning << p_error_node->get_location() << " : "
                     << "edit_box : font_string's anchors will be ignored." << std::endl;
        }

        if (const layout_node* p_error_node = defaulted.try_get_child("Size")) {
            gui::out << gui::warning << p_error_node->get_location() << " : "
...
void edit_box::parse_text_insets_node_(const layout_node& node) {
    if (const layout_node* p_text_insets_node = node.try_get_child("TextInsets")) {
