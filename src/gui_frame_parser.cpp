    ...
    if (const layout_node* pResizeBoundsNode = mNode.try_get_child("ResizeBounds")) {
        if (const layout_node* pMinNode = pResizeBoundsNode->try_get_child("Min")) {
            ...
            if (mDimensions.first == anchor_type::ABS) {
                if (bHasX && bHasY) {
                    set_min_dimensions(
                        vector2f(mDimensions.second.x.value(), mDimensions.second.y.value()));
                } else if (bHasX)
...
void frame::parse_backdrop_node_(const layout_node& mNode) {
    if (const layout_node* pBackdropNode = mNode.try_get_child("Backdrop")) {
        ...
        pBackdrop->set_background(
            parse_file_name(pBackdropNode->get_attribute_value_or<std::string>("bgFile", "")));

        pBackdrop->set_edge(
            parse_file_name(pBackdropNode->get_attribute_value_or<std::string>("edgeFile", "")));

        pBackdrop->set_background_tilling(
            pBackdropNode->get_attribute_value_or<bool>("tile", false));
        ...
        if (const layout_node* pEdgeInsetsNode = pBackdropNode->try_get_child("EdgeInsets")) {
            ...
            if (pAbsInsetNode) {
                ...
                gui::out << gui::warning << pRelInsetNode->get_location() << " : "
                         << "RelInset for Backdrop:EdgeInsets is not yet supported (" << sName_
                         << ")." << std::endl;
            }
        }
        ...
        if (const layout_node* pEdgeSizeNode = pBackdropNode->try_get_child("EdgeSize")) {
            ...
            if (pAbsValueNode && pRelValueNode) {
                gui::out
                    << gui::warning << pEdgeSizeNode->get_location()
                    << " : "
        ...
        if (const layout_node* pTileSizeNode = pBackdropNode->try_get_child("TileSize")) {
            ...
            if (pAbsValueNode && pRelValueNode) {
                gui::out
                    << gui::warning << pTileSizeNode->get_location()
                    << " : "
...
void frame::parse_hit_rect_insets_node_(const layout_node& mNode) {
    if (const layout_node* pHitRectBlock = mNode.try_get_child("HitRectInsets")) {
        ...
        if (pAbsInsetNode && pRelInsetNode) {
            gui::out
                << gui::warning << pHitRectBlock->get_location()
                << " : "
...
utils::observer_ptr<layered_region> frame::parse_region_(
    const layout_node& mNode, const std::string& sLayer, const std::string& sType) {
    try {
        auto mAttr = parse_core_attributes(
            get_manager().get_root().get_registry(),
            get_manager().get_virtual_root().get_registry(), mNode, observer_from(this));
        ...
        for (const layout_node& mLayerNode : pLayersNode->get_children()) {
            if (mLayerNode.get_name() != "Layer" && mLayerNode.get_name() != "") {
                gui::out << gui::warning << mLayerNode.get_location() << " : "
                         << "unexpected node '" << mLayerNode.get_name() << "'; ignored."
                         << std::endl;
                continue;
            }
...
utils::observer_ptr<frame> frame::parse_child_(const layout_node& mNode, const std::string& sType) {
    try {
        auto mAttr = parse_core_attributes(
            get_manager().get_root().get_registry(),
            get_manager().get_virtual_root().get_registry(), mNode, observer_from(this));
