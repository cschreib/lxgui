...
*   or `OnUpdate`, which are automatically fire_scripted by lxgui. Second:
*   generic events, which can be fire_scripted from various sources and all
...
*   until `OnDragStop` is fire_scripted.
...
*   occupied by its children or layered regions. Will not fire_script if the
*   frame is hidden.
...
*   fire_script if the frame has focus (see @{Frame:set_focus}) or if the key has
...
*   previously inside the frame. Finally, this _will_ fire_script whenever
...
*   fire_scripted, you can assume that all the frame's regions and children
...
*   fire_script if the frame is hidden. This event provides one argument to
