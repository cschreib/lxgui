...
void renderer::set_view_(const matrix4f& mViewMatrix) {
    ...
    float fScaleX =
        std::sqrt(mViewMatrix(0, 0) * mViewMatrix(0, 0) + mViewMatrix(1, 0) * mViewMatrix(1, 0));
    float fScaleY =
        std::sqrt(mViewMatrix(0, 1) * mViewMatrix(0, 1) + mViewMatrix(1, 1) * mViewMatrix(1, 1));
    float fAngle =
        std::atan2(mViewMatrix(0, 1) / fScaleY, mViewMatrix(0, 0) / fScaleX) * RAD_TO_DEG;
...
matrix4f renderer::get_view() const {
    matrix4f mCurrentViewMatrix =
        matrix4f(pCurrentSFMLTarget_->getView().getTransform().getMatrix());
    if (!pCurrentTarget_) {
...
sf::Transform to_sfml(const matrix4f& mMatrix) {
    return sf::Transform(
        mMatrix(0, 0), mMatrix(1, 0), mMatrix(3, 0), mMatrix(0, 1), mMatrix(1, 1), mMatrix(3, 1),
        0.0, 0.0, 1.0);
}
...
std::shared_ptr<gui::material> renderer::create_material(
    ...
    std::shared_ptr<sfml::material> pTex =
        std::make_shared<sfml::material>(mDimensions, false, material::wrap::REPEAT, mFilter);
