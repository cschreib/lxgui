...
render_target::render_target(const vector2ui& mDimensions, material::filter mFilter) {
    pTexture_ =
        std::make_shared<sfml::material>(mDimensions, true, material::wrap::REPEAT, mFilter);
...
void render_target::clear(const color& mColor) {
    pRenderTexture_->clear(
        sf::Color(mColor.r * 255, mColor.g * 255, mColor.b * 255, mColor.a * 255));
}
