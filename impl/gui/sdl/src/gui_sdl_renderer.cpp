...
renderer::renderer(SDL_Renderer* pRenderer, bool bInitialiseSDLImage) : pRenderer_(pRenderer) {
    ...
    SDL_Texture* pTexture = SDL_CreateTexture(
        pRenderer_, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, 4u, 4u);

    bPreMultipliedAlphaSupported_ =
        (SDL_SetTextureBlendMode(
             pTexture, (SDL_BlendMode)material::get_premultiplied_alpha_blend_mode()) == 0);
...
ub32color to_ub32color(const color& mColor) {
    return {
        static_cast<unsigned char>(mColor.r * 255), static_cast<unsigned char>(mColor.g * 255),
        static_cast<unsigned char>(mColor.b * 255), static_cast<unsigned char>(mColor.a * 255)};
}
...
sdl_render_data
make_rects(const std::array<vertex, 4>& lVertexList, float fTexWidth, float fTexHeight) {
    ...
    int iWidth =
        static_cast<int>(std::round(lVertexList[lIDs[2]].pos.x - lVertexList[lIDs[0]].pos.x));
    int iHeight =
        static_cast<int>(std::round(lVertexList[lIDs[2]].pos.y - lVertexList[lIDs[0]].pos.y));
    int iUVIndex1 = 0;
    int iUVIndex2 = 2;
    ...
    mData.mSrcQuad = SDL_Rect{
        ...
        (int)std::round(
            (lVertexList[lIDs[iUVIndex2]].uvs.x - lVertexList[lIDs[iUVIndex1]].uvs.x) * fTexWidth),
        (int)std::round(
            (lVertexList[lIDs[iUVIndex2]].uvs.y - lVertexList[lIDs[iUVIndex1]].uvs.y) *
            fTexHeight)};
...
void renderer::render_quad_(const sdl::material* pMat, const std::array<vertex, 4>& lVertexList) {
    ...
    if (pMat) {
        ...
        if (mData.mDestQuad.w == 0 || mData.mDestQuad.h == 0)
            return;
        ...
        if (lViewList[0].col == lViewList[1].col && lViewList[0].col == lViewList[2].col &&
            ...
            if (bPreMultipliedAlphaSupported_) {
                SDL_SetTextureColorMod(
                    pTexture, mColor.r * mColor.a * 255, mColor.g * mColor.a * 255,
                    mColor.b * mColor.a * 255);
            } else {
            ...
            if (pMat->get_wrap() == material::wrap::CLAMP || bCoordsAllInTexture || bOnePixel) {
                ...
                SDL_RenderCopyEx(
                    pRenderer_, pTexture, &mData.mSrcQuad, &mData.mDestQuad, mData.iAngle,
                    &mData.mCenter, mData.mFlip);
            } else {
                ...
                while (iSY < mData.mSrcQuad.y + mData.mSrcQuad.h) {
                    ...
                    while (iSX < mData.mSrcQuad.x + mData.mSrcQuad.w) {
                        ...
                        SDL_RenderCopyEx(
                            pRenderer_, pTexture, &mSrcQuad, &mDestQuad, mData.iAngle,
                            &mData.mCenter, mData.mFlip);
        ...
        if (lViewList[0].col == lViewList[1].col && lViewList[0].col == lViewList[2].col &&
            ...
            if (bPreMultipliedAlphaSupported_) {
                SDL_SetRenderDrawBlendMode(
                    pRenderer_, (SDL_BlendMode)material::get_premultiplied_alpha_blend_mode());
                SDL_SetRenderDrawColor(
                    pRenderer_, mColor.r * mColor.a * 255, mColor.g * mColor.a * 255,
                    mColor.b * mColor.a * 255, mColor.a * 255);
            } else {
                SDL_SetRenderDrawBlendMode(pRenderer_, SDL_BLENDMODE_BLEND);
                SDL_SetRenderDrawColor(
                    pRenderer_, mColor.r * 255, mColor.g * 255, mColor.b * 255, mColor.a * 255);
            }
            ...
            for (int y = 0; y < mDestQuad.h; ++y)
                for (int x = 0; x < mDestQuad.w; ++x) {
                    const color lColY1 =
                        interpolate_color(lColorQuad[0], lColorQuad[3], y / float(mDestQuad.h - 1));
                    const color lColY2 =
                        interpolate_color(lColorQuad[1], lColorQuad[2], y / float(mDestQuad.h - 1));
                    pPixelData[y * mDestQuad.w + x] =
                        to_ub32color(interpolate_color(lColY1, lColY2, x / float(mDestQuad.w - 1)));
                }
...
void renderer::render_quads_(
    const gui::material* pMaterial, const std::vector<std::array<vertex, 4>>& lQuadList) {
...
renderer::create_material_(const std::string& sFileName, material::filter mFilter) {
...
std::shared_ptr<gui::material> renderer::create_material(
    const vector2ui& mDimensions, const ub32color* pPixelData, material::filter mFilter) {
...
std::shared_ptr<gui::material> renderer::create_material(
    ...
    if (mLocation == pRenderTarget->get_rect()) {
        ...
        return std::make_shared<sdl::material>(
            pRenderer_, pTex->get_render_texture(), mLocation, pTex->get_filter());
    }
}

std::shared_ptr<gui::render_target>
renderer::create_render_target(const vector2ui& mDimensions, material::filter mFilter) {
...
std::shared_ptr<gui::font> renderer::create_font_(
    ...
    return std::make_shared<sdl::font>(
        pRenderer_, sFontFile, uiSize, uiOutline, lCodePoints, uiDefaultCodePoint,
        bPreMultipliedAlphaSupported_);
}
