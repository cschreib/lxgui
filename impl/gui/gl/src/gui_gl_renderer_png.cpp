...
renderer::create_material_png_(const std::string& sFileName, material::filter mFilter) const {
    ...
    if (!mFile.good() || png_sig_cmp(lSignature, 0, PNGSIGSIZE) != 0) {
        throw gui::exception(
            "gui::gl::manager", sFileName + "' is not a valid PNG image : '" +
    ...
    try {
        ...
        gui::out << gui::error << "gui::gl::manager : Error parsing " << sFileName << "."
                 << std::endl;
