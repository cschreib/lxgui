...
namespace gui
{
void button::register_glue(utils::wptr<lua::state> pLua)
{
    pLua->reg<lua_button>();
}
...
{
    pButtonParent_ = dynamic_cast<button*>(pParent_);
...
int lua_button::_set_disabled_font_object(lua_State* pLua)
{
    ...
    if (mFunc.check())
    {
        ...
        if (plua_font_string)
        {
            font_string* pFontString = dynamic_cast<font_string*>(plua_font_string->get_parent());
