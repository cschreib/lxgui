...
struct anchor_data {
    ...
    anchor_data(
        anchor_point mInputPoint, const std::string& sInputParent, anchor_point mInputParentPoint) :
        mPoint(mInputPoint), sParent(sInputParent), mParentPoint(mInputParentPoint) {}

    anchor_data(
        anchor_point       mInputPoint,
        const std::string& sInputParent,
        anchor_point       mInputParentPoint,
        const vector2f&    mInputOffset,
        anchor_type        mInputType = anchor_type::ABS) :
        mPoint(mInputPoint),
        sParent(sInputParent),
        mParentPoint(mInputParentPoint),
